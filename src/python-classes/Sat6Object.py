...
class Sat6Object(object): #: pylint: disable=useless-object-inheritance
    ...
    def lookup_lce_name(self, lce_tag):
        ...
        '''
        self.logger.debug('Entering Function: %s', sys._getframe().f_code.co_name) #: pylint: disable=protected-access
        self.logger.debug('Looking for lce: %s', lce_tag)

        if 'lce' not in self.lutables:
            self.logger.debug('First time calling function, loading table.')
            ...
            if results['success']:
                ...
                if '_revision' in self.lutables['lce']:
                    self.logger.debug('LCE Table revision: %s', self.lutables['lce']['_revision'])
                else:
                    self.logger.debug('Warning: LCE Table did not have _revision tag.')
            else:
        ...
        if hostname is None:
            ...
            if not isinstance(hostname, int):
                ...
                if self.results['success']:
                    ...
                    self.logger.debug('find unsuccessful: %s', self.results)
                    hostname = None
            if hostname is not None:
                self.results = self.util.get_item('%s/hosts/%s' % (self.foreman, hostname),
                                                  'host_id %s' % hostname)

        self.logger.debug('get_host: %s', self.results['msg'])
        if self.results['success']:
    ...
    def get_cv(self, cview=None):
        ...
        if cview is None:
            ...
            self.logger.debug('Looking for cview: %s', cview)
            ...
            if cview is not None:
                self.results = self.util.get_item('%s/content_views/%s' % (self.katello, cview),
                                                  'cview_id %s' % cview)

        self.logger.debug('get_cv: %s', self.results['msg'])
        if self.results['success']:
            return self.results['return']
        return None
    ...
    def get_hc(self, collection=None):
        ...
        if collection is None:
            ...
            if collection is not None:
                self.results = self.util.get_item('%s/host_collections/%s' % (self.katello, collection),
                                                  'collection_id %s' % collection)

        self.logger.debug('get_hc: %s', self.results['msg'])
        if self.results['success']:
            return self.results['return']
        return None
    ...
    def get_org(self, organization=None):
        ...
        if organization is None:
            ...
            self.logger.debug('Looking for organization: %s', organization)

            if not isinstance(organization, int):
                self.results = self.util.find_item('%s/organizations' % (self.katello),
                                                   organization)
                if self.results['success']:
            ...
            if organization is not None:
                self.results = self.util.get_item('%s/organizations/%s' % (self.katello, organization),
                                                  'org_id %s' % organization)

        self.logger.debug('get_org: %s', self.results['msg'])
        if self.results['success']:
            return self.results['return']
        return None
    ...
    def get_org_lce(self, lce_name, org_id=None):
        ...
        if lce_name is None:
            ...
            self.logger.debug('Looking for Life Cycle Environment %s in org %s.', lce_name, org_id)

            if not isinstance(lce_name, int):
                self.results = self.util.find_item('%s/organizations/%s/environments' % (self.katello, org_id),
                                                   lce_name)
                if self.results['success']:
            ...
            if lce_name is not None:
                self.results = self.util.get_item('%s/organizations/%s/environments/%s' % (self.katello, org_id, lce_name),
                                                  'lce_id %s' % lce_name)

        self.logger.debug(self.results['msg'])
        if self.results['success']:
            return self.results['return']
        return None
    ...
    def get_org_lce_list(self, search=None, field='name', org_id=None):
        ...
        if org_id is None:
            org_id = self.org_id
        self.logger.debug('Retriveing list of Lifecycle Environments for org_id %s.', org_id)
    ...
    def get_loc(self, location=None):
        ...
        if location is None:
            ...
            self.logger.debug('Looking for location: %s', location)
            ...
            if not isinstance(location, int):
                ...
                self.results = self.util.find_item('%s/locations/' % (self.foreman),
                                                   location, field)
                if self.results['success']:
            ...
            if location is not None:
                self.results = self.util.get_item('%s/locations/%s' % (self.foreman, location),
                                                  'loc_id %s' % location)
    ...
    def set_host_cv(self, host=None, cview=None):
       ...
       '''
        ...
        if host is None:
            ...
            self.logger.debug('Host does not have ID attribute, attempting lookup for: %s.', host)
            host = self.get_host(host)
        #-- We rely on the fact that get_host will set self.results appropriately
        if self.results['success'] is False:
            self.logger.debug('set_host_cv: %s', self.results['msg'])
            return False
        ...
        if cview is None:
            ...
            self.logger.debug('CView does not have ID attribute, attempting lookup for: %s.', cview)
    ...
    def set_host_lce(self, host, lce):
       ...
       '''
        ...
        #-- We rely on the fact that get_host will set self.results appropriately
        if self.results['success'] is False:
            self.logger.debug('set_host_lce: %s', self.results['msg'])
            return False
        if 'content_facet_attributes' not in host:
            self.results['success'] = False
            self.results['msg'] = '%s is not a content host.' % (host['name'])
        ...
        #-- We rely on the fact that get_org_lce will set self.results appropriately
            ...
            self.logger.debug('LCE does not have ID attribute, attempting lookup for: %s.', lce)
    ...
    def set_host_loc(self, host, location):
       ...
       '''
        ...
        #-- We rely on the fact that get_host will set self.results appropriately
        if self.results['success'] is False:
            self.logger.debug('set_host_loc: %s', self.results['msg'])
            return False
        ...
        #-- We rely on the fact that get_org_location will set self.results appropriately
            ...
            self.logger.debug('Location does not have ID attribute, attempting lookup for: %s.', location)
    ...
    def add_host_hc(self, host, collection):
       ...
       '''
        ...
        #-- We rely on the fact that get_host will set self.results appropriately
        if self.results['success'] is False:
            self.logger.debug('add_host_hc: %s', self.results['msg'])
            return False
        ...
        #-- We rely on the fact that get_org_location will set self.results appropriately
            ...
            self.logger.debug('Collection does not have ID attribute, attempting lookup for: %s.', collection)
